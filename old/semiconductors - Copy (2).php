...
<form action="javascript:sendFrm()" name="reciever">Add a Part:<table border=0>
...
if(isset($_POST['com'])){ //check for Add/Update/Remove
  if($_POST['com']=="add"){
    ...
    $values=explode('^',$_POST['val']);
    echo "added";
    $sql="insert into semiconductors_trans (quantity,number,type,Package,Description,datasheet) values ($values[0],'".urldecode($values[1])."','$values[2]','$values[3]',\"".urldecode($values[4])."\",\"".urldecode($values[5])."\");";
    mysql_query($sql) or die(mysql_error()."\n".$sql);
  }
}//endif AUR
?>
